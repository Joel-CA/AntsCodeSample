    ...
    def add_to(self, place):
        ...
        if place.ant is None:
           place.ant = self
            ...
            if self.can_contain(place.ant): #new ant is cointainer
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        place_counter = 0
        while not place_tracker.is_hive:#check all the places until we reach the bee-hive
            if place_tracker.bees == [] or not(self.lower_bound <= place_counter <= self.upper_bound): #if there aren't bees in this place, or not in range...
                place_tracker = place_tracker.entrance #check next place
                place_counter += 1
            else:
                return random_bee(place_tracker.bees) #otherwise, select one of the bees (randomly)
        return None
...
class FireAnt(Ant):
    ...
    def reduce_health(self, amount):
        ...
        # BEGIN Problem 5
        "*** YOUR CODE HERE ***"
        print("DEBUG: self:", self, "self.place: ", self.place)
        place = self.place
        super().reduce_health(amount)#already removes ant if dead
        if self.health <= 0:
...
# The WallAnt class
class WallAnt(Ant):
    """WallAnt has buffed health to stall incoming bees"""
...
# The HungryAnt Class
class HungryAnt(Ant):
    """HungryAnt can instantly kill any bee by eating it; however, it takes 3 turns to chew where it cannot do damage :("""
...
class TankAnt(ContainerAnt):
    """TankAnt provides protection to other Ants AND does +1 damage to every ant in its place."""
...
class ScubaThrower(ThrowerAnt):
    """ScubaThrower is a type of ThrowerAnt which is capable of surviving on water places"""
    food_cost = 6
    implemented = True
    is_waterproof = True
    name = 'Scuba'
    ...
    def action(self, gamestate):
        ...
        while place_tracker:
            if place_tracker.ant:
                if place_tracker.ant.is_container and place_tracker.ant.ant_contained:
                    place_tracker.ant.ant_contained.double()
                place_tracker.ant.double()
...
class SlowThrower(ThrowerAnt):
    ...
    def __init__(self, health=1):
        super().__init__(health)
    # SlowThrower throws sticky syrup at a bee, slowing it for 5 turns. When a bee is slowed,
    # it takes the regular Bee action when gamestate.time is even, and takes no action 
    # (does not move or sting) otherwise. If a bee is hit by syrup while it is already slowed,
    # it is slowed for 5 turns starting from the most recent time it is hit by syrup. That is,
    # if a bee is hit by syrup, takes 2 turns, and is hit by syrup again, it will be slowed for
    # 5 turns after the second time it is hit by syrup
    def throw_at(self, target):
        #Applies a temporary effect on the action method of a Bee instance that it calls throw_at on.
        # BEGIN Problem EC
        "*** YOUR CODE HERE ***"
        self.turns_till_effect = 5
        
        def act(gamestate):
            if target != None:
                if self.turns_till_effect > 0: # if it is slowed...
                    if gamestate.time % 2 == 0:
                        Bee.action(target, gamestate)
                    self.turns_till_effect -=1
                else:
                    Bee.action(target, gamestate)
        target.action = act
            #self.nearest_bee().action = Bee.action
        #self.throw_at(self.nearest_bee())
        # END Problem EC
